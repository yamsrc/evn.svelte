...
Given('new account', async ({ page }) => {
  await page.goto('/')
  await page.locator('[data-marker="email-tab"]').click()
  await expect(page.locator('[data-marker="username-input"]')).toBeFocused()
  await page.keyboard.type(faker.internet.email())
  await page.keyboard.press('Tab')
  await page.keyboard.type(faker.internet.password())
  await page.keyboard.press('Enter')

  const nameInput = page.locator('[data-marker="name-input"]')
